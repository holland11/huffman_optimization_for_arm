	...
	*/
	...
	while (1) {
		...
		if (index == 0 || index == 6) {
			...
			// height 25 subtree but we have a lookup table up to height 15 (12 bits after we already took the first 3)
			table_index = readNBits(12, &bits_left_in_buffer, &buffer, f_in);
			node = tree[lookup_tables[index][table_index]];
			if (!node.left && !node.right) {
				// leaf node so we don't need to continue the traversal
				c = (char)node.letter;
				if (node.nbits < 15) storeNBitsBack(15-node.nbits, (u_int64_t)table_index, &buffer, &bits_left_in_buffer);
			} else {
				while (1) {
