...
int fillTreeNodeArray(Node* node, ArrayNode arr[], int* index, int current, int depth, int bit_string, u_int8_t lookup[], int max_height, int table_lookup_builder, int depth_count[]) {
	...
	if (table_lookup_builder && node->left == NULL && node->right == NULL) {
		...
		if (depth > 15) { // we dont want to capture depth 16+ from subtree 7
		} else {
			print32binary(bit_string);
			printf("node %u bits %d index %u is char %u\n", result, depth, bit_string, node->letter);
	...
	} else if (table_lookup_builder && depth == 15) {
		// depth is 15 but it's not a leaf node. we still need to record this one
		print32binary(bit_string);
		printf("node %u bits %d index %u is internal node\n", result, depth, bit_string);
		lookup[bit_string] = result;
	}
	if (node->left != NULL) {
		tempL = fillTreeNodeArray(node->left, arr, index, 0, depth+1, bit_string << 1, lookup, max_height, table_lookup_builder, depth_count);
	}
	if (node->right != NULL) {
		tempR = fillTreeNodeArray(node->right, arr, index, 0, depth+1, (bit_string << 1) + 1, lookup, max_height, table_lookup_builder, depth_count);
	}
...
void traverseSubTrees(Node** tree_nodes, int* index) {
	...
	*/
	...
	u_int8_t* lookup[] = {temp0, temp1, temp2, temp3, temp4, temp5, temp6, temp7};
	int max_height[] = {0, 1, 1, 12, 3, 3, 0, 12}; 
	ArrayNode arr[248];
	int table_lookup_builder = 1;
	int depth_count[29];
	for (i=0; i<8; i++) {
		/*if (order[i] == 7 || order[i] == 3) {  
			for (j=0; j<29; j++) {
				depth_count[j] = 0;
			}
			table_lookup_builder = 0;
		}*/
		int root = order[i];
		printf("\n\ntraversing subtree %d\n", order[i]);
		int temp = fillTreeNodeArray(tree_nodes[root], arr, index, root+1, 3, 0, lookup[order[i]], max_height[order[i]], table_lookup_builder, depth_count);
		
		/*if (order[i] == 7 || order[i] == 3) {
			printf("\n subtree %d depth count\n", order[i]);
			for (j=0; j<29; j++) {
				printf("%d: %d\n", j, depth_count[j]);
			}
			table_lookup_builder = 1;
		}*/
	}
	printf("\n");
	for (i=0; i < 1; i++) {
